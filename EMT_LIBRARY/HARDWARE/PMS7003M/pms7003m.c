...
u16 ERR_CODE;
u16 recv_uart4_byte_count;


#if EN_UART4_RX   //如果使能了接收

u8  UART4_RX_BUF[UART4_REC_LEN]; //接收缓冲,最大USART_REC_LEN个字节.末字节为换行符 
...
void UART4_IRQHandler(void)                	//串口1中断服务程序
{
	...
	OSIntEnter();    
#endif
	if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x42 0x4d开头)
	{
			Res =USART_ReceiveData(UART4);	//读取接收到的数据
			if((UART4_RX_STA&0x8000)==0)//接收未完成
			{
				if(UART4_RX_STA&0x4000)//接收到了0x42 0x4d
				{
						UART4_RX_BUF[recv_uart4_byte_count]=Res ;
						recv_uart4_byte_count ++;
					  if (recv_uart4_byte_count==32)
						{				  
							for(i=0;i<32;i++)
							{
								pms7003_recv_buf[i]=UART4_RX_BUF[i];
							}
							PMS7003MConvert();
							UART4_RX_STA|=0x8000; //接受完；
						}
				...
				{	
					...
					{
					   if (Res==0x42)	UART4_RX_STA|=0x2000;
						 else 
							 UART4_RX_STA=0;
					}		 
				}
...
{  
  ...
  sendcmd=0;
	do
	{
		for(i=0;i<7;i++)
		{
			USART_SendData(UART4,pms7003_cmd_buf[i]);
			while(USART_GetFlagStatus(UART4, USART_FLAG_TXE) == RESET){} //waiting send ok
		}
		delay_us(200);
		sendcmd++;
	}while(sendcmd<5);	
