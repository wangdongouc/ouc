...
u8 ESP8266_STA_TCPCLient(void)
{
 ...
 /**
  * @file   ESP8266_STA_UDP_Init
  * @brief  ESP8266ÉèÖÃSTAÄ£Ê½ÏÂUDP
  * @param  ÎÞ
  * @retval 0---8
  */
u8 ESP8266_STA_UDP_Init(void)
{
  ...
  printf("ESP8266ÉèÖÃUDPÖÐ... ...\r\n");
  res=ESP8266_Set_UDP(SERVER_IP,SERVER_PORT,LOCAL_PORT,500); //ÉèÖÃUDP,²¢½øÈëÍ¸´«£¬³¬Ê±Ê±¼ä5s£¬Ê§°Ü·µ»Ø7	
  switch(res)
  {
    case 0 : break;
    case 1 : return 7;    //ÉèÖÃUDPÊ§°Ü£¬·µ»Ø7
    case 2 : return 8;    //½øÈëÍ¸´«Ê§°Ü£¬·µ»Ø8
  }
...
u8 ESP8266_Set_UDP(u8 *ip, u16 port, u16 local_port, u16 timeout)
{
	...
	USART2_RX_STA=0;                        
	memset(USART2_RX_BUF,0,USART2_REC_LEN);                                //Çå³ý´®¿Ú2Êý¾Ý  
	u2_printf("AT+CIPSTART=\"UDP\",\"%s\",%d,%d,0\r\n",ip,port,local_port);//·¢ËÍUDPÉèÖÃÃüÁî
	time=0;
	while(time<timeout)
	{
		if(strstr( (const char *)USART2_RX_BUF , (const char *)"OK"))//Èç¹û½ÓÊÜµ½OK±íÊ¾³É¹¦
			break;
		delay_ms(100);//ÑÓÊ±100ms
		time++;      
	}
	...
	else  //×¼±¸½øÈëÍ¸´«
	{
		...
		if(time>=timeout)return 2;                //Í¸´«³¬Ê±´íÎó£¬·µ»Ø4
		else return 0;	                          //³É¹¦·µ»Ø0		
	}
}
 /**
